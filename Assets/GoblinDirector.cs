...
public class GoblinDirector : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown("Fire2"))
        {
            ...
            {//gets the location clicked on and stores it in hit
                for (int i = 0; i < players.Length; i++)
                {//iterates through all the players 
                    GameObject gameObject = players[i];
                    if (gameObject.GetComponent<GoblinMove>().getIsSelected())
                    {
                        gameObject.GetComponent<GoblinMove>().setTarget(hit);//sets the target location for all players that are selected
                    }
                }
            }
        }
        ...
        if (Input.GetButtonDown("Fire1"))
        {
            if (Physics.Raycast(ray, out hit))
            {
                ...
                {
                    if (hit.transform.gameObject.GetComponent<GoblinMove>().getIsSelected())
                    {
                        hit.transform.gameObject.GetComponent<GoblinMove>().isNowNotSelected();
                    }
                    else
                    {
                        hit.transform.gameObject.GetComponent<GoblinMove>().isNowSelected();
                    }
        ...
        if (Input.GetButtonDown("Deselect"))
        {
            ...
            {
                GameObject gameObject = players[i];
                gameObject.GetComponent<GoblinMove>().isNowNotSelected();
            }
        }
    }
}
